...
public class CreateSession extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.isNew()) {
			...
			out.println("<h3>SESSION CREATED FOR " + ((String)session.getAttribute("username")).toUpperCase() + "!</h3>");
		} else {
			out.print("<h4>Session already active for current user: " + session.getAttribute("username") + "</h4>");
		}
