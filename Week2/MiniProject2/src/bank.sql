...
SELECT a.USERNAME, a.PASSWORD, b.STATUS, c.AMOUNT 
FROM LOGIN a NATURAL INNER JOIN STATUS b 
NATURAL INNER JOIN ACCOUNT c;

DELETE LOGIN WHERE USERNAME = 'harish';
ALTER TABLE 
   Login 
MODIFY 
   ( 
...
;
CREATE OR REPLACE PROCEDURE updateUser(in_username IN varchar2)
IS
BEGIN
    UPDATE LOGIN 
    SET STATUSID = 1
    ...
    INSERT INTO ACCOUNT 
    VALUES((SELECT USERID FROM LOGIN WHERE USERNAME = in_username),0.0); 
    commit;
END;
/

call UPDATECUSTOMER('world', 100.55);

UPDATE ACCOUNT SET AMOUNT = 100.50 WHERE USERID = (SELECT USERID FROM LOGIN WHERE USERNAME = 'Harish');

CREATE OR REPLACE PROCEDURE updateCustomer(in_username IN varchar2, in_amount IN NUMBER)
IS
BEGIN
    UPDATE ACCOUNT 
    SET AMOUNT = in_amount 
    WHERE USERID = (SELECT USERID FROM LOGIN WHERE USERNAME = in_username); 
    commit;
END;
/
