...
public class EditReimbursement extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			while(it.hasNext()) { // iterate through the list while there are still elements within the list of FileItems.
				...
				if(!file.isFormField()) {
					...
					else if(file.getFieldName().equals("reiid")) {rei_id = Integer.parseInt(file.getString());} 		//	Rei_id
		...
		String lname = (String) session.getAttribute("lname");
			...
			if(DataService.editReimbursement(rei_id, empid, fname, lname, dateOf, time, location,
					desc, cost, gradingFormat, typeOfEvent, work, attachment_bit) == 1) {
				HTMLTemplates.headers(out);
